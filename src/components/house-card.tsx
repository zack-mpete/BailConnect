...
import { houseContractHref, houseDetailHref } from "@/lib/house-links";
...
export function HouseCard({ house }: { house: House }) {
  ...
  const contractHref = user ? houseContractHref(house, user) : loginHref(houseContractHref(house));
      ...
      <Card className="overflow-hidden p-0">
        ...
        <div className="space-y-4 p-4">
          ...
          <div className="flex gap-2">
            <Button href={detailHref} className="flex-1 bg-ink text-white">Voir détails</Button>
            <Button href={contractHref} className="bg-brand-50 text-brand-700">{user ? "Contrat" : "Se connecter"}</Button>
          </div>
        </div>
      </Card>
    </article>
